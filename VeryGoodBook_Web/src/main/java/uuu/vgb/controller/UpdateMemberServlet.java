 ...
 */
@WebServlet(urlPatterns="/member/update.do")//http://localhost:8080/vgb/member/update.do
public class UpdateMemberServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, 
		...
		if(userid==null || !userid.equals(member.getId())) {
			errorsList.add("不得修改帳號");
		}
		
		if(password==null || !password.equals(member.getPassword())) {
			errorsList.add("密碼不正確");
		}
		...
		if(errorsList.isEmpty()) {			
			try {
				Customer c = member.getClass().getConstructor().newInstance();
				c.setId(member.getId());
				c.setName(name);
				c.setPassword(changePassword!=null?password1:member.getPassword());
				c.setEmail(email);
